(*
   ...
   2014-12-02 22:19:37
     将编码，和解码改成使用TdxMemoeryStream，有效利用内存池
*)
unit uDIOCPDxStreamCoder;
...
var
  ...
  Stream: TDxMemoryStream;
begin
  Stream := TDxMemoryStream.Create(MB_SPLarge);
  ...
  Stream.WriteBuffer(lvPACK_FLAG,2);
  ...
  Stream.WriteBuffer(lvVerifyValue,SizeOf(lvVerifyValue));
  lvWriteIntValue := TByteTools.swap32(lvDataLen);
  Stream.WriteBuffer(lvWriteIntValue, SizeOf(lvWriteIntValue));
  //Stream.WriteStream(TStream(pvDataObject),lvDataLen);
  Stream.WriteBuffer(lvbuf[0],lvDataLen);
  Stream.LinkToBufferList(ouBuf);
  //ouBuf.readBuffer(@lvPACK_FLAG,2);
  //if lvPACK_FLAG <> PACK_FLAG then
  Stream.Free;
  //pack_flag
