...
uses
  ...
  Dialogs, iocpBaseSocket, StdCtrls, ExtCtrls, uRunTimeINfoTools;
...
type
  ...
  private
    FIocpSocket: TIocpBaseSocket;

    { Private declarations }
  public
    class function createAsChild(pvParent: TWinControl; pvIocpSocket: TIocpBaseSocket): TFMMonitor;

    property IocpSocket: TIocpBaseSocket read FIocpSocket write FIocpSocket;
  end;
...
class function TFMMonitor.createAsChild(pvParent: TWinControl; pvIocpSocket:
    TIocpBaseSocket): TFMMonitor;
begin
  Result := TFMMonitor.Create(pvParent.Owner);
  Result.Parent := pvParent;
  Result.Align := alClient;
  Result.IocpSocket := pvIocpSocket;
